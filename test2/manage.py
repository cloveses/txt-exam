...
class TestHandler(tornado.web.RequestHandler):
    ...
    def gen_seq(self):
        ...
        s_seq=self.gen_aseq(testnum[0])
        f_seq=self.gen_aseq(testnum[1])
        y_seq=self.gen_aseq(testnum[2])
        sid=int(self.get_cookie('sid'))
        TestInfo(sid=sid,test_status='started',s_seq=s_seq,f_seq=f_seq,y_seq=y_seq).save()
        # print(s_seq)
        # print(f_seq)
        # print(y_seq)

    def gen_aseq(self,tnum):
        from random import shuffle
        seq=[i for i in range(1,tnum+1)]
        shuffle(seq)
        return seq
...
class GetTestHandler(tornado.web.RequestHandler):
    def get(self,test_type,tno='1'):
        if self.get_cookie('sid'):
            aTest=Test.objects(test_type=int(test_type),tno=self.get_tno_num(test_type,int(tno))).first()
            htmlstr=''
            if aTest:
                htmlstr=self.get_html_str(test_type,aTest,tno)
    ...
    def get_tno_num(self,test_type,tno):
        sid=int(self.get_cookie('sid'))
        a_ti=TestInfo.objects(sid=sid).first()
        if a_ti:
            if test_type=='1':
                return a_ti.s_seq[tno-1]
            elif test_type=='2':
                return a_ti.f_seq[tno-1]
            elif test_type=='3':
                return a_ti.y_seq[tno-1]
        else:
            return 1
    ...
    def get_html_str(self,test_type,aTest,tno):
        from testnum import testnum
        if int(test_type)==1:
            html_str_lst=[
                ...
                r'<input name="test_no" type="hidden" value="',str(tno),r'" />',
                '<p>',str(tno),'. ',aTest.test_content,'</p>']
            if aTest.image_url:
